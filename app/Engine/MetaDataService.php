...
class MetaDataService
{
     ...
     */
    private static $baseApiFolder = HOME . 'app/Resources/';
    ...
    public static function loadHelpDocumentation(string $service, string $requestMethod)
    {
        ...
        // load path
        $path = HOME . 'app/Verb/' . strtoupper($requestMethod) . '/' . $requestMethod . $service . '.md';
    ...
    private static function parseMarkdownToHTML(string $path, string $service, string $requestMethod) : void
    {
        ...
        // Load template
        $template = file_get_contents(HOME . '/src/public/style/template.html');
        
        // change title
        $template = str_replace('{TITLE}', $requestMethod . ucfirst($service) . '::service', $template);

        // add css link
        $template = str_replace('{LINK}', func()->url('src/public/style/markdown.css'), $template);
        $template = str_replace('{LINK2}', func()->url('src/public/style/highlight.css'), $template);

        // add javascript link
        $template = str_replace('{JAVASCRIPT}', func()->url('src/public/style/highlight.js'), $template);
